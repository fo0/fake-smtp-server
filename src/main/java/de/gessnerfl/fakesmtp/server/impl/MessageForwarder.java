...
public class MessageForwarder {
  ...
  @Autowired
  public MessageForwarder(FakeSmtpConfigurationProperties configurationProperties,
                          JavaMailSenderFacade javaMailSenderFacade, Logger logger) {
  ...
  public void forward(RawData rawData) {
    logger.debug("evaluate message: {}", rawData);
    if (configurationProperties.isForwardEmails()) {
      ...
      try {
        ...
        logger.warn(
            "Failed to convert raw data to MimeMessage; fall back to simple message forwarding",
            e);
